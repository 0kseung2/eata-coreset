...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, scale_factor=2, num_samples_update=0, coreset_size=64):
    ...
    if current_model_probs is not None: 
        ...
        coreset_ids = center_based_distance_coreset(features[filter_ids_1][filter_ids_2], k) # 두 번 필터링 된 샘플들에 대해서 코어셋으로 한 번 더 추출
        ...
        coreset_ids = center_based_distance_coreset(features[filter_ids_1], k) # 한 번 필터링 된 샘플들에 대해서 코어셋으로 한 번 더 추출
        entropys = entropys[coreset_ids]
...
def center_based_distance_coreset(features, k):
    mu = features.mean(dim=0)
    dists = torch.norm(features - mu, dim=1)
    idx = torch.argsort(dists)[:k]
    # PyTorch에서 [:k]는 슬라이싱이기 때문에, k가 인덱스 개수보다 커도 자동으로 가능한 범위만큼만 반환됨
    return idx
