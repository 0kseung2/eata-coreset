...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, scale_factor=2, num_samples_update=0, coreset_size=64):
    ...
    if current_model_probs is not None: 
        ...
        coreset_ids = euclidean_herding_coreset(features[filter_ids_1][filter_ids_2], k) # 두 번 필터링 된 샘플들에 대해서 코어셋으로 한 번 더 추출
        ...
        coreset_ids = euclidean_herding_coreset(features[filter_ids_1], k) # 한 번 필터링 된 샘플들에 대해서 코어셋으로 한 번 더 추출
        entropys = entropys[coreset_ids]
...
def euclidean_herding_coreset(features, k):
    """
    유클리디안 기반 herding coreset 알고리즘
    """
    ...
    for t in range(1, k+1):
        ...
        target = t * mu - (t - 1) * mu_t
        diff = features[remaining] - target
        scores = torch.norm(diff, dim=1)  # Euclidean distance
        min_idx = torch.argmin(scores)   # 거리 가장 작은 샘플 선택
        true_idx = torch.nonzero(remaining, as_tuple=False)[min_idx].item()
