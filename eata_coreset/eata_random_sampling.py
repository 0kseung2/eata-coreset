...
class EATA(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, fishers=None, fisher_alpha=2000.0, steps=1, episodic=False, e_margin=math.log(1000)/2-1, d_margin=0.05, filtering_size=64):
        super().__init__()
        self.model = model
        ...
        self.total_filtering_samples = 0
        self.total_filtering_sample_per_corruption = 0
    ...
    def forward(self, x):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, num_counts_2, num_counts_1, updated_probs, filtering_samples_num = forward_and_adapt_eata(x, self.model, self.optimizer, self.fishers, self.e_margin, self.current_model_probs, fisher_alpha=self.fisher_alpha, num_samples_update=self.num_samples_update_2, d_margin=self.d_margin, filtering_size=self.filtering_size)
                self.num_samples_update_2 += num_counts_2
                self.num_samples_update_1 += num_counts_1
                self.reset_model_probs(updated_probs)
                self.total_filtering_samples += filtering_samples_num # 필터링 샘플 개수 누적
                self.total_filtering_sample_per_corruption += filtering_samples_num # 필터링 샘플 개수 누적
        else:
            self.model.eval()
            with torch.no_grad():
...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, scale_factor=2, num_samples_update=0, filtering_size=64):
    ...
    if fishers is not None:
        ...
        loss += ewc_loss
    if x[ids1][ids2][random_filtering_ids].size(0) != 0:
        loss.backward()
        optimizer.step()
    optimizer.zero_grad()
    return outputs, entropys.size(0), filter_ids_1[0].size(0), updated_probs, random_filtering_samples_num

def random_filtering_indices(n, k):
