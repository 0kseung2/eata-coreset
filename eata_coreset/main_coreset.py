...
import eata_coreset_dot_product
import eata_coreset_center_based_distance
import eata_coreset_euclidean
...
def get_args():
    ...
    parser.add_argument('--algorithm', default='eata_coreset_center_based_distance', type=str, help='eata_coreset_center_based_distance or eata_coreset_euclidean or eata_coreset_dot_product')  
    parser.add_argument('--coreset_size', default=64, type=int, help='coreset size (default: 64)')
...
if __name__ == '__main__':
    ...
    if args.algorithm == 'eata_coreset_center_based_distance':
        ...
        subnet = eata_coreset_center_based_distance.configure_model(subnet)
        params, param_names = eata_coreset_center_based_distance.collect_params(subnet)
        ...
        adapt_model = eata_coreset_center_based_distance.EATA(subnet, optimizer, fishers, args.fisher_alpha, e_margin=args.e_margin, d_margin=args.d_margin, coreset_size=args.coreset_size)
    elif args.algorithm == 'eata_coreset_dot_product':
        # compute fisher informatrix
        args.corruption = 'original'
        fisher_dataset, fisher_loader = prepare_test_data(args)
        fisher_dataset.set_dataset_size(args.fisher_size)
        ...
        subnet = eata_coreset_dot_product.configure_model(subnet)
        params, param_names = eata_coreset_dot_product.collect_params(subnet)
        ewc_optimizer = torch.optim.SGD(params, 0.001)
        fishers = {}
        train_loss_fn = nn.CrossEntropyLoss().cuda()
        for iter_, (images, targets) in enumerate(fisher_loader, start=1):      
        ...
        optimizer = torch.optim.SGD(params, 0.00025, momentum=0.9)
        adapt_model = eata_coreset_dot_product.EATA(subnet, optimizer, fishers, args.fisher_alpha, e_margin=args.e_margin, d_margin=args.d_margin, coreset_size=args.coreset_size)
    elif args.algorithm == 'eata_coreset_euclidean':
        # compute fisher informatrix
        args.corruption = 'original'
        fisher_dataset, fisher_loader = prepare_test_data(args)
        fisher_dataset.set_dataset_size(args.fisher_size)
        ...
        subnet = eata_coreset_euclidean.configure_model(subnet)
        params, param_names = eata_coreset_euclidean.collect_params(subnet)
        ewc_optimizer = torch.optim.SGD(params, 0.001)
        fishers = {}
        train_loss_fn = nn.CrossEntropyLoss().cuda()
        for iter_, (images, targets) in enumerate(fisher_loader, start=1):      
        ...
        optimizer = torch.optim.SGD(params, 0.00025, momentum=0.9)
        adapt_model = eata_coreset_euclidean.EATA(subnet, optimizer, fishers, args.fisher_alpha, e_margin=args.e_margin, d_margin=args.d_margin, coreset_size=args.coreset_size)
    else:
        assert False, NotImplementedError
    ...
    for corrupt in common_corruptions:
        ...
        logger.info(f"From {corrupt}, total_coreset_sample_per_corruption: {adapt_model.total_coreset_sample_per_corruption}") # 손상별 코어셋 샘플 개수 누적
        adapt_model.total_coreset_sample_per_corruption = 0 # 손상별 코어셋 샘플 개수 리셋
        logger.info(f"total_coreset_samples: {adapt_model.total_coreset_samples}") # 전체 코어셋 샘플 개수 누적, 나중에 역전파에서의 flops 계산할 때 사용
